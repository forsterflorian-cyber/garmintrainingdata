...
def build_review_prompt(review_package: Dict[str, Any]) -> str:
    return (
        ...
        "Context:\n"
        "This is a personal training decision dashboard for a recreational but performance-oriented hybrid athlete.\n"
        "The review should NOT use an overly conservative risk-averse coaching standard.\n"
        "Moderate and threshold work are acceptable when recovery is reasonably supportive and load is not clearly excessive.\n"
        "Only mark a decision as \"too_aggressive\" when there are clear negative indicators that make the recommended intensity meaningfully inappropriate.\n"
        "Do NOT default to easy/recovery just because a safer option exists.\n"
        "Judge whether the recommendation is appropriate for balanced progression, not maximal caution.\n\n"
        "Evaluation principles:\n"
        "- \"correct\" means the recommendation is reasonable for balanced training progression.\n"
        "- \"too_conservative\" means the model held back more than necessary.\n"
        "- \"too_aggressive\" means the model recommended more intensity than was reasonably justified.\n"
        "- Prefer \"correct\" unless there is a clear reason to disagree.\n"
        "- Elevated recent hard-session density alone is NOT enough to make a recommendation too aggressive.\n"
        "- Threshold can still be acceptable when recovery is decent, load ratio is acceptable, and there are no strong red flags.\n"
        "- Missing data should only lead to \"insufficient_data\" when the missing field is critical to the decision.\n\n"
        "Use this scale in spirit:\n"
        "- recovery/easy: only when recovery or fatigue context clearly calls for restraint\n"
        "- moderate: acceptable on many normal training days\n"
        "- threshold: acceptable when recovery is supportive and fatigue is manageable\n"
        "- vo2: should require clearly strong support\n\n"
